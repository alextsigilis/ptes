...
%% Define Directories
fprintf("Where are the edf files stored?\n");
datadir = uigetdir("");

fprintf("Where to store the images?\n");
savedir = uigetdir("");
cd(datadir);
...
if isfile(dsfile)
    ...
    sz = [0 4];
    names = ["idx", "epoch", "path", "annotation"];
...
for pid = 1 : 154
    ...
    for i = 1 : size(bic,1)
        ...
        t = table([idx], [i], [path], [annotation], 'VariableNames',names);
        dataset = [dataset; t];
        imwrite(b,path);
    end
